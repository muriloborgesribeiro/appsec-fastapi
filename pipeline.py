...
def verificar_dependencias():
    ...
    for modulo, (nome, descricao) in deps.items():
        try:
            ...
            print(
                f"       [ERRO] {nome} NAO encontrado -- instale com: pip install {nome}"
            )
            todas_ok = False
    ...
    try:
        ...
        print(
            f"       [OK] Orange3 {versao} (tecnologia da disciplina -- validacao visual)"
        )
    except ImportError:
        print(
            "       [AVISO] Orange3 nao instalado -- workflow .ows sera gerado mas requer Orange3 para abrir"
        )
...
def baixar_dataset():
    ...
    try:
        ...
        if os.path.exists(raw_path):
            ...
            print(
                "         Baixe manualmente de: https://archive.ics.uci.edu/dataset/938"
            )
...
def preprocessar_dados(raw_path):
    ...
    if configs_risco:
        print(
            f"       [RISCO] Configs com Alvarado_Score: {configs_risco} (excluidas da selecao)"
        )
    ...
    for cfg_id in configs_testar:
        ...
        print(
            f"       Etapa 0: {resultado['n_removidos_target']} removidas (target NaN)"
        )
        print(
            f"       Camada 1: {resultado['n_removidos_obrig']} removidas (features obrigatorias NaN)"
        )
        if resultado["n_imputados_opcionais"] > 0:
            print(
                f"       Camada 2: {resultado['n_imputados_opcionais']} valores imputados"
            )
            ...
            resultado.get("detalhes_nan_opcionais", {}).get("linhas_descartadas", 0) > 0
        ):
            print(
                f"       Camada 2: {resultado['detalhes_nan_opcionais']['linhas_descartadas']}"
                f" linhas descartadas (opcionais sem valor)"
            )
        print(
            f"       Features: {len(resultado['features'])} | "
        ...
        resultado_treino = treinar_knn(resultado["X_train"], resultado["y_train"])

        # Avaliar no teste
        modelo = resultado_treino["modelo"]
        acuracia_teste = float(modelo.score(resultado["X_test"], resultado["y_test"]))
        acuracia_cv = resultado_treino["resultados_cv"][resultado_treino["k"]][
            "acuracia_media"
        ]

        print(
            f"       k={resultado_treino['k']} | CV={acuracia_cv:.1%} | Teste={acuracia_teste:.1%}"
        )
        print()
    ...
    print("       ============================================")
    print(
        f"       {'Config':<8} {'Status':<12} {'Feat':<6} {'N_treino':<10} {'N_teste':<10} {'k':<5} {'CV':<10} {'Teste':<10}"
    )
    print(f"       {'-' * 71}")
    ...
    for cfg_id in configs_testar:
        ...
        print(
            f"       {cfg_id:<8} {status:<12} {n_feat:<6} {r['n_treino']:<10} {r['n_teste']:<10} "
    ...
    print(
        f"       >>> SELECIONADA: Config {melhor_cfg} "
    ...
    if melhor_acc >= 0.80:
        ...
        print(
            f"       Causa: Acuracia limitada pelo tamanho do dataset -- "
...
def treinar_modelo(dados_processados):
    ...
    if os.path.exists(modelo_path):
        ...
        print(
            f"       Config={dados_modelo.get('config', '?')} | "
    ...
    for k_val, res in resultado_treino["resultados_cv"].items():
        ...
        print(
            f"       {marcador} k={k_val}: acuracia={res['acuracia_media']:.1%} "
    ...
    metricas = {
        ...
        "acuracia_cv": round(
            resultado_treino["resultados_cv"][resultado_treino["k"]]["acuracia_media"],
            4,
        ),
...
def treinar_svm(dados_processados):
    ...
    if os.path.exists(modelo_path):
        ...
        dados_modelo = joblib.load(modelo_path)
        print(
            f"       kernel={dados_modelo.get('kernel', '?')} | "
    ...
    resultado_treino = _treinar_svm(
        dados_processados["X_train"], dados_processados["y_train"]
    )

    # Avaliar no teste
    modelo = resultado_treino["modelo"]
    acuracia_teste = float(
        modelo.score(dados_processados["X_test"], dados_processados["y_test"])
    )

    # Mostrar cross-validation
    print(
        f"       Melhor config: kernel={resultado_treino['kernel']}, C={resultado_treino['C']}"
    )
    for chave, res in resultado_treino["resultados_cv"].items():
        marcador = (
            ">>>"
            if (
                res["kernel"] == resultado_treino["kernel"]
                and res["C"] == resultado_treino["C"]
            )
            else "   "
        )
        print(
            f"       {marcador} {chave}: acuracia={res['acuracia_media']:.1%} "
            f"(+/-{res['desvio_padrao']:.1%})"
        )
...
def avaliar_modelo(dados_processados, dados_modelo_knn, dados_modelo_svm):
    ...
    metricas_knn = _avaliar(
        y_real,
        y_pred_knn,
        output_dir=img_dir,
        nome_modelo="KNN",
        nome_arquivo="confusion_matrix_knn.png",
    )

    print(
        f"       KNN:  VP={metricas_knn['vp']}  FP={metricas_knn['fp']}  "
        f"FN={metricas_knn['fn']}  VN={metricas_knn['vn']}"
    )
    print(
        f"       [OK] KNN Sensibilidade: {metricas_knn['sensibilidade']:.1%} | "
    ...
    metricas_svm = _avaliar(
        y_real,
        y_pred_svm,
        output_dir=img_dir,
        nome_modelo="SVM",
        nome_arquivo="confusion_matrix_svm.png",
    )

    print(
        f"       SVM:  VP={metricas_svm['vp']}  FP={metricas_svm['fp']}  "
        f"FN={metricas_svm['fn']}  VN={metricas_svm['vn']}"
    )
    print(
        f"       [OK] SVM Sensibilidade: {metricas_svm['sensibilidade']:.1%} | "
    ...
    roc_knn = gerar_curva_roc(
        y_real, y_prob_knn, os.path.join(img_dir, "roc_knn.png"), "KNN"
    )
    pr_knn = gerar_curva_pr(
        y_real, y_prob_knn, os.path.join(img_dir, "pr_knn.png"), "KNN"
    )
    curvas_info["knn"] = {**roc_knn, **pr_knn}
    print(
        f"       [OK] KNN  -- AUC-ROC: {roc_knn['auc_roc']:.3f} | "
    ...
    roc_svm = gerar_curva_roc(
        y_real, y_prob_svm, os.path.join(img_dir, "roc_svm.png"), "SVM"
    )
    pr_svm = gerar_curva_pr(
        y_real, y_prob_svm, os.path.join(img_dir, "pr_svm.png"), "SVM"
    )
    curvas_info["svm"] = {**roc_svm, **pr_svm}
    print(
        f"       [OK] SVM  -- AUC-ROC: {roc_svm['auc_roc']:.3f} | "
    ...
    if y_pred_alvarado is not None:
        ...
        if mask_valid.sum() > 10:
            ...
            roc_alv = gerar_curva_roc(
                y_real_alv,
                y_prob_alv,
                os.path.join(img_dir, "roc_alvarado.png"),
                "Alvarado",
            )
            pr_alv = gerar_curva_pr(
                y_real_alv,
                y_prob_alv,
                os.path.join(img_dir, "pr_alvarado.png"),
                "Alvarado",
            )
            curvas_info["alvarado"] = {**roc_alv, **pr_alv}
            print(
                f"       [OK] Alvarado -- AUC-ROC: {roc_alv['auc_roc']:.3f} | "
            ...
            y_prob_knn_alv = modelo_knn.predict_proba(
                dados_processados["X_test"].iloc[mask_valid]
            )[:, 1]
            ...
            modelos_comp = [
                {
                    "nome": "KNN",
                    "y_real": y_real_alv,
                    "y_prob": y_prob_knn_alv,
                    "cor": "#1565c0",
                },
                {
                    "nome": "SVM",
                    "y_real": y_real_alv,
                    "y_prob": y_prob_svm_alv,
                    "cor": "#6a1b9a",
                },
                {
                    "nome": "Alvarado",
            ...
            print(
                "       [AVISO] Poucos scores Alvarado validos -- curva Alvarado ignorada"
            )
    ...
    print(
        f"       [OK] Curvas comparativas salvas em diagnostico/static/diagnostico/img/"
    )
    print()
...
def atualizar_metricas_completas(
    metricas_knn, metricas_svm, metricas_alvarado, dados_modelo_svm, curvas_info=None
):
    """
    ...
    if metricas_alvarado is not None:
        metricas["avaliacao_alvarado"] = {
            ...
            "total": metricas_alvarado["vp"]
            + metricas_alvarado["fp"]
            + metricas_alvarado["fn"]
            + metricas_alvarado["vn"],
    ...
    if curvas_info:
        ...
        metricas["referencia_roc"] = (
            "Fawcett T. (2006). DOI:10.1016/j.patrec.2005.10.010"
        )
    ...
    print(
        f"       [OK] metricas.json atualizado com avaliacao_knn, "
...
def resumo_final(
    metricas_knn, metricas_svm, metricas_alvarado, dados_modelo_knn, dados_modelo_svm
):
    """
    ...
    print(f"  Sensibilidade:     {metricas_knn['sensibilidade']:.1%}  (VP/(VP+FN))")
    print(f"  Especificidade:    {metricas_knn['especificidade']:.1%}  (VN/(VN+FP))")
    print(f"  VPP:               {metricas_knn['vpp']:.1%}  (VP/(VP+FP))")
    print(f"  VPN:               {metricas_knn['vpn']:.1%}  (VN/(VN+FN))")
    print()
    print(
        f"  Resultados do Modelo SVM (kernel={dados_modelo_svm['kernel']}, C={dados_modelo_svm['C']})"
    )
    print(f"  -----------------------------------------")
    print(f"  Acuracia final:    {metricas_svm['acuracia']:.1%}")
    print(f"  Sensibilidade:     {metricas_svm['sensibilidade']:.1%}  (VP/(VP+FN))")
    print(f"  Especificidade:    {metricas_svm['especificidade']:.1%}  (VN/(VN+FP))")
    print(f"  VPP:               {metricas_svm['vpp']:.1%}  (VP/(VP+FP))")
    print(f"  VPN:               {metricas_svm['vpn']:.1%}  (VN/(VN+FN))")
    print()
    if metricas_alvarado:
        print(f"  Alvarado Score (corte >= 5):")
        print(f"  -----------------------------------------")
        print(
            f"  Sensibilidade:     {metricas_alvarado['sensibilidade']:.1%}  (VP/(VP+FN))"
        )
        print(
            f"  Especificidade:    {metricas_alvarado['especificidade']:.1%}  (VN/(VN+FP))"
        )
    ...
    print("  [3] Databases     -> Banco de dados SQLite + ORM")
...
def start_server(args):
    ...
    port = args.port
    reload = args.reload
    print(f"Iniciando servidor APPSPEC em http://{host}:{port}")
...
def main():
    parser = argparse.ArgumentParser(
        description="APPSPEC — Sistema de Apoio ao Diagnóstico de Apendicite",
    )
    parser.add_argument(
        "command",
        nargs="?",
        default="setup",
        choices=["setup", "start"],
        help="'setup' executa o pipeline ML (padrão); 'start' inicia o servidor web",
    )
    parser.add_argument(
        "--host", default="0.0.0.0", help="Host do servidor (padrão: 0.0.0.0)"
    )
    parser.add_argument(
        "--port", type=int, default=8082, help="Porta do servidor (padrão: 8082)"
    )
    parser.add_argument(
        "--reload",
        action="store_true",
    ...
    atualizar_metricas_completas(
        metricas_knn, metricas_svm, metricas_alvarado, dados_modelo_svm, curvas_info
    )
